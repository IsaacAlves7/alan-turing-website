...
function EvaluateStep(symbol, id){
    ...
    if(symbols[symbol] == undefined) return { Error : true, Acceptable : Machines[id].Functions.Acceptable(Machines[id].State), Message : `O símbolo <strong>'${symbol}'</strong> não tem transação definida no estado <strong>${Machines[id].State}</strong> dessa máquina. ${symbols.ERROR ? symbols.ERROR : ''}`};
...
function Evaluate(id){
        ...
        if(!Machines[id].Functions.Acceptable(Machines[id].State) && chain.length > i && Machines[id].Count < 10000){
            ...
            if(Machines[id].Functions.Acceptable(Machines[id].State)){
                Update();            
                Stop();
                $('#modalTitle').html('Corrente válida.');
                $('#modalText').html('A corrente inserida é válida. Pode ver o resultado na gravação.');
                $('#myModal').modal('show');
            ...
            if(Machines[id].Count >= 10000){
                ...
                Stop();
                $('#modalTitle').html('Erro!');
                $('#modalText').html('A corrente inserida ela gerou muitas transições sem definir um resultado.');
                $('#myModal').modal('show');
            }
        }
...
function SetMachine1(){
    Machines[1][0] = {
        ...
        '+' : ['+', 0, 1],
        '=' : ['=', 3, 1],
        "ERRO" : "A máquina diz: corrente vazia , o formato está incorreto."
    };
    ...
    Machines[1][4] = {
        '1' : ['1', 1, 1],
        "ERRO" : "A máquina diz: não existe símbolo unário depois do símbolo operador."
    };
...
function SetMachine2(){
    Machines[2][0] = {
        '1' : ['1', 14, 1],
        "ERRO" : "A máquina diz: não existe símbolo unário antes do símbolo operador."
    }
    ...
    Machines[2][3] = {
        '1' : ['X', 4, 1],
        '=' : ['=', 11, 1],
        "ERRO" : "A máquina diz: não se esperava um segundo operador."
    }
    ...
    Machines[2][11] = {
        '1' : ['1', 13, 1],
        '-' : ['-', 13, 1],
        "ERRO" : "A máquina diz:subtração não é definida entre dois números iguais."
    }
...
function SetMachine5(){
    ...
    Machines[5][0] = {
        '1' : ['X', 1, 1],
        "ERRO" : "A máquina diz: se esperava um símbolo unário como início de corrente."
    }
